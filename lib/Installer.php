...
class Installer {
	...
	function connect_to_db($hostname, $username, $password) {
		...
		if($conn){
			...
			$this->addError("Database connect failed[".mysql_error()."]");
			return false;
		}
		return $conn;
	}
	...
	function install($hostname, $user, $pwd, $dbname, $admname, $admpwd,$ihavedb,$sqlfile,$webroot,$lan) {
		...
		if(!$info){
			$this->addError("Please check whether $dbname is exist");
			return false;
		}else{
		...
		$this->write2cfg($hostname, $user, $pwd, $dbname,$webroot,$lan);
