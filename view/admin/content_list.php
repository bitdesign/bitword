										...
										<tbody>
											<?
												foreach ($arrayList as $row){
													...
													echo "<tr><td><input type='checkbox' name='tyChecks' value='".$row["id"]."'/></td><td>".$row["block_name"]."</td><td class='ctx-title'>".$row["title"]."</td><td>".$content_sts[$row["sts"]]."</td><td>".btime($row["edit_tm"])."</td><td><a title=\"$tdelete\" onclick=del(".$row["id"].")><i class='fa fa-fw fa-trash-o'></i></a>&nbsp;&nbsp;<a title=\"$tedit\" onclick=edit('".$row["id"]."')><i class='fa fa-pencil-square-o'></i></a>&nbsp;&nbsp;<a title=\"$torecommend\" onclick=recommend('".$row["id"]."','1')><i class='fa fa-star'></i></a></td></tr>";
													else
													echo "<tr><td><input type='checkbox' name='tyChecks' value='".$row["id"]."'/></td><td>".$row["block_name"]."</td><td class='ctx-title'>".$row["title"]."</td><td>".$content_sts[$row["sts"]]."</td><td>".btime($row["edit_tm"])."</td><td><a title=\"$tdelete\" onclick=del(".$row["id"].")><i class='fa fa-fw fa-trash-o'></i></a>&nbsp;&nbsp;<a title=\"$tedit\" onclick=edit('".$row["id"]."')><i class='fa fa-pencil-square-o'></i></a>&nbsp;&nbsp;<a title=\"$torecommend\" onclick=recommend('".$row["id"]."','')><i class='fa fa-star-empty'></i></a></td></tr>";
												}
											?>
										</tbody>
									</table>
